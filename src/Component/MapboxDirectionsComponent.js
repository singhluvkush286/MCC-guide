...
const MapboxDirectionsComponent = ({ origin, destination }) => {
  ...
  useEffect(() => {
   ...
   const createArrowElement = () => {
    ...
    svg.innerHTML = `
      <path fill="#000000" d="M0 0h24v24H0z" fill-opacity="0"/>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div ref={mapContainerRef} style={{ width: '100%', height: '400px', marginBottom: '10px' }} />

      {/* Display the user's current location */}
      <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 1 }}>
        <button onClick={() => geolocateControlRef.current.trigger()}>Show My Location</button>
      </div>

      {/* Display directions information */}
      {instructions.length > 0 && (
        <div style={{ width: '100%', maxWidth: '400px', padding: '10px', border: '1px solid #ccc' }}>
